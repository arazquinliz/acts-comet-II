...
// ---------------------------------------------------------------- //
/* 
    This program performs a realistic track fitting procedure. It uses 
    ...
    Command:
        g++ finalFit.cpp Geometry.cpp MagField.cpp Measurements.cpp 
        KalmanFitting.cpp Logger.cpp Seeder.cpp -o finalFit.x
...
int main() {
	...
	pOptions.mass                    = 0.511_MeV; // 1000 by default
	...
	for (size_t i = 0; i < event_vector.size(); i++) {
		...
		int sgn = p_estim > 0 ? 1 : -1;
		double p     = std::abs(p_estim)* 0.001; // mom.norm() * 0.001;  //  
		double phi   = params[Acts::eBoundPhi]; // sign * acos(mom.x() / sqrt(mom.x() * mom.x() + mom.y() * mom.y())); //  
		double theta = params[Acts::eBoundTheta]; // acos(mom.z() / sqrt(mom.x() * mom.x() + mom.y() * mom.y() + mom.z() * mom.z())); // //  0.142523 0.775937; 
		int    q     = -1;
