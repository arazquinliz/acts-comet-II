...
// ---------------------------------------------------------------- //
/* 
    This program performs full track reconstruction with a set of input 
    spacepoints. It contains the standard basic structure
	for running CKF with unknown initial seeds and multiple trajectories.
	The fitted momentum, original momentum and fit chi2 and ndf are outputed
	in recon_mom.txt, which later on can be analysed with recon_stats.c in root.
    ...
    The seeded triplets are also stored in seeded_meas.txt so that they can be
    checked.

    Command:
        g++ trackRecon.cpp Geometry.cpp MagField.cpp Seeder.cpp 
        KalmanFitting.cpp -o recon.x
...
int main() {
    ...
    for (int eventID = 0; eventID < 2000; eventID++) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (size_t j = 0; j < seeds.size(); j++) {
                ...
                //if ((abs(p_estimA) < 40) || (abs(p_estimA) > 200)) {std::cout << "MOMENTUM CUT" << std::endl;continue;}
                ...
                if (i == 0) {
                    ...
                    double time  = 0.;
                    /*double dX    = seed[1].x() - seed[0].x();
                    double dY    = seed[1].y() - seed[0].y();
                    double dZ    = seed[1].z() - seed[0].z();*/
                    // This angles should be of the estimated direction
                    double phi   = paramsA[Acts::eBoundPhi]; // dY/abs(dY) * atan(dX / std::sqrt(dX * dX + dY * dY));
                    double theta = paramsA[Acts::eBoundTheta]; // acos(dZ / std::sqrt(dX * dX + dY * dY + dZ * dZ));
